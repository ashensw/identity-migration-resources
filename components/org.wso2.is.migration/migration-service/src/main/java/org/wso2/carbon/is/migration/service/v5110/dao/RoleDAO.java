...
public class RoleDAO {
    ...
    public static final String RETRIEVE_EXTERNAL_ROLE_DATA =
            "SELECT DISTINCT(UM_ROLE_PERMISSION.UM_ROLE_NAME), UM_ROLE_PERMISSION.UM_DOMAIN_ID, UM_DOMAIN"
                    ...
                    + " UM_DOMAIN.UM_TENANT_ID AND UM_DOMAIN.UM_DOMAIN_NAME NOT IN (:HYBRID_DOMAINS;)";
     ...
     */
    public List<RoleInfo> getExternalRoleData(Connection connection) throws SQLException {
    ...
    /**
     * Method to assign a new group to the given role.
     *
     ...
     * @throws SQLException SQLException.
     */
    public void updateGroupListOfRole(Connection connection, String roleName, String groupName, int tenantId)
            throws SQLException {
        ...
        try (NamedPreparedStatement statement = new NamedPreparedStatement(connection, addGroupsSQL)) {
     ...
     * @throws SQLException SQLException.
     */
    public void transferPermissionsOfRole(Connection connection, RoleInfo roleInfo) throws SQLException {

        try (NamedPreparedStatement statement = new NamedPreparedStatement(connection, UPDATE_ROLE_NAME_SQL)) {
            connection.setAutoCommit(false);
            statement.setString(NEW_UM_ROLE_NAME, roleInfo.getInternalRoleName());
            statement.setString(UM_ROLE_NAME, roleInfo.getRoleName());
            statement.setString(INTERNAL_DOMAIN, UserCoreConstants.INTERNAL_DOMAIN.toUpperCase(Locale.ENGLISH));
            statement.setInt(UM_TENANT_ID, roleInfo.getTenantID());
            statement.setInt(UM_DOMAIN_ID, roleInfo.getDomainID());
            statement.executeUpdate();
            connection.commit();
        }
    }
